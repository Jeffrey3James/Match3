  ...
  public class Gem : MonoBehaviour {
    ...
    public void Select() {
      ...
      pulseTween = transform.DOScale(.85f, .3f)
          ...
          .SetLink(gameObject);   // auto-kill when this gem is destroyed
      }
    ...
    // Guarantee no tween survives the GameObject.
    // Fixes: DOTween MissingReferenceException on RectTransform when a
    // selected/pulsing gem is destroyed by a cascade or power-up.
    void OnDestroy() {
      pulseTween?.Kill();
      pulseTween = null;
      transform.DOKill();
